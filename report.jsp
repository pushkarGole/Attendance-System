...
<html>
        ...
        <c:choose>
            <c:when test="${param.attend_type=='theory'}">
                ...
                <c:forEach var="prsnt_stu" items="${paramValues.attend}">
                    <sql:update dataSource="${db}">
                        update attendance_1 set theory_attend=theory_attend+? where student_id=? and subject_id=? and Date=?;
                        <sql:param value="${param.num_class}"></sql:param>
    ...
    <body onload='attedn_sucess()'>
        <script type="text/javascript">
            ...
            <form action="faculty_home.jsp" method="POST" id="form1">
                <input type="hidden" name="user_id" value="${param.user_id}"/>
                <input type="hidden" name="password" value="${param.password}"/>
                <input type="submit" hidden="hidden" id="sbmbtn"/>
            </form>
                <sql:update dataSource="${db}">
                    update teacher set logged_in=0 where user_id=? and password=?;
